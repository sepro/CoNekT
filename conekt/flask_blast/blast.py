...
class BlastThread(Thread):
    ...
    def init_app(self, app):
        ...
        if not self.running and (
            ...
            or os.environ.get("WERKZEUG_RUN_MAIN") is None
            or not app.config["DEBUG"]
        ):
    ...
    def add_job(self, blast_type, blast_input, blast_output):
        ...
        """
        job = {"type": blast_type, "in": blast_input, "out": blast_output}
    ...
    def __process_job(self, job):
        ...
        if job["type"] == "blastp" or job["type"] == "blastn":
            command = (
                self.commands[job["type"]]
                .replace("<IN>", job["in"])
                .replace("<OUT>", job["out"] + ".tmp")
            )
            ...
            print(
                ' * Blast tread : job type "' + job["type"] + '" unknown !',
                file=sys.stderr,
            )
