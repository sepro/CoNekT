...
class CompareSpecificityForm(FlaskForm):
    ...
    def populate_form(self):
        self.family_method.choices = [
            (m.id, m.method)
            for m in GeneFamilyMethod.query.order_by(GeneFamilyMethod.id)
        ]
        self.speciesa.choices = [(0, "Select species")] + [
        ...
        self.speciesb.choices = [(0, "Select species")] + [
            (s.id, s.name) for s in Species.query.order_by(Species.name)
        ]
