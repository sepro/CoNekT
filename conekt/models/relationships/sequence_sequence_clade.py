...
class SequenceSequenceCladeAssociation(db.Model):
    ...
    sequence_one_id = db.Column(
        db.Integer, db.ForeignKey("sequences.id", ondelete="CASCADE")
    )
    sequence_two_id = db.Column(
        db.Integer, db.ForeignKey("sequences.id", ondelete="CASCADE")
    )

    clade_id = db.Column(
        db.Integer, db.ForeignKey("clades.id", ondelete="CASCADE"), index=True
    )
    tree_id = db.Column(
        db.Integer, db.ForeignKey("trees.id", ondelete="CASCADE"), index=True
    )
    ...
    tree = db.relationship(
        ...
        lazy="joined",
        backref=db.backref(
            "sequence_sequence_clade_associations", lazy="dynamic", passive_deletes=True
        ),
    )

    clade = db.relationship(
        "Clade",
        lazy="joined",
        backref=db.backref(
            "sequence_sequence_clade_associations", lazy="dynamic", passive_deletes=True
