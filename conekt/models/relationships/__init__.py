...
sequence_family = db.Table(
    ...
    db.Column("sequence_id", db.Integer, db.ForeignKey("sequences.id"), index=True),
    db.Column(
        "gene_family_id", db.Integer, db.ForeignKey("gene_families.id"), index=True
    ),
)

sequence_coexpression_cluster = db.Table(
...
coexpression_cluster_similarity = db.Table(
    "coexpression_cluster_similarity",
    db.Column("id", db.Integer, primary_key=True),
    db.Column(
        "source_id", db.Integer, db.ForeignKey("coexpression_clusters.id"), index=True
    ),
    db.Column(
        "target_id", db.Integer, db.ForeignKey("coexpression_clusters.id"), index=True
    ),
)

sequence_xref = db.Table(
    "sequence_xref",
    db.Column("id", db.Integer, primary_key=True),
    db.Column("sequence_id", db.Integer, db.ForeignKey("sequences.id"), index=True),
...
sequence_sequence_ecc = db.Table(
    "sequence_sequence_ecc",
    db.Column("id", db.Integer, primary_key=True),
...
sequence_sequence_clade = db.Table(
    "sequence_sequence_clade",
    db.Column("id", db.Integer, primary_key=True),
...
cluster_go_enrichment = db.Table(
    "cluster_go_enrichment",
    db.Column("id", db.Integer, primary_key=True),
    db.Column(
        "cluster_id", db.Integer, db.ForeignKey("coexpression_clusters.id"), index=True
    ),
    db.Column("go_id", db.Integer, db.ForeignKey("go.id"), index=True),
)


cluster_clade_enrichment = db.Table(
    "cluster_clade_enrichment",
    db.Column("id", db.Integer, primary_key=True),
    db.Column(
        "cluster_id", db.Integer, db.ForeignKey("coexpression_clusters.id"), index=True
    ),
