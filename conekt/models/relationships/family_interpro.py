...
class FamilyInterproAssociation(db.Model):
    ...
    interpro_id = db.Column(
        db.Integer, db.ForeignKey("interpro.id", ondelete="CASCADE")
    )

    gene_family = db.relationship(
        "GeneFamily",
        backref=db.backref(
            "interpro_annotations", lazy="dynamic", passive_deletes=True
        ),
        lazy="joined",
    )
    ...
    domain = db.relationship(
        "Interpro",
        backref=db.backref("family_associations", lazy="dynamic", passive_deletes=True),
        lazy="joined",
    )
