...
class ExpressionSpecificityMethod(db.Model):
    ...
    condition_tissue = db.relationship(
        "ConditionTissue",
    ...
    @staticmethod
    def calculate_tissue_specificities(
        species_id,
        description,
        condition_to_tissue,
        order,
        ...
        for profile_id, profile in profiles:
            ...
            profile_specificities = sorted(
                profile_specificities, key=lambda x: x["score"], reverse=True
            )
            ...
            if len(specificities) > 400:
                db.engine.execute(
                    ExpressionSpecificity.__table__.insert(), specificities
                )
                specificities = []
...
class ExpressionSpecificity(db.Model):
    ...
    profile_id = db.Column(
        db.Integer,
        db.ForeignKey("expression_profiles.id", ondelete="CASCADE"),
        index=True,
    )
