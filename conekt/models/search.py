...
class Search:
    ...
    def simple(term_string):
        ...
        sequences = (
            Sequence.query.filter(
                or_(
                    ...
                    and_(
                        *[
                            Sequence.description.ilike("%" + term + "%")
                            for term in terms
                            if len(term) > 3
                        ]
                    ),
        ...
        go = (
            GO.query.filter(
                or_(
                    and_(*[GO.description.ilike("%" + term + "%") for term in terms]),
        ...
        families = (
            GeneFamily.query.filter(func.upper(GeneFamily.name).in_(terms))
            .limit(50)
            .all()
        )
        profiles = (
            ExpressionProfile.query.filter(ExpressionProfile.probe.in_(terms))
            .limit(50)
            .all()
        )
    ...
    def whooshee_simple(term_string):
        ...
        sequences_by_name = (
            Sequence.query.filter(Sequence.name.in_(terms)).limit(50).all()
        )

        sequences_by_xref = (
            Sequence.query.filter(Sequence.xrefs.any(XRef.name.in_(terms)))
            .limit(50)
            .all()
        )
        ...
        # that are too short
        whooshee_search_terms = [
            t for t in re.sub("(\W|\d)+", " ", term_string).split() if len(t) > 3
        ]
        ...
        if (
            all([len(t) == 0 for t in [sequences, go, interpro, families, profiles]])
            and len(whooshee_search_string) > 3
        ):
            # didn't find a term by ID, try description
            whooshee_go = GO.query.whooshee_search(
                whooshee_search_string, limit=50
            ).all()
            whooshee_sequences = Sequence.query.whooshee_search(
                whooshee_search_string, limit=50
            ).all()
            whooshee_interpro = Interpro.query.whooshee_search(
                whooshee_search_string, limit=50
            ).all()

        return {
            "go": (go + whooshee_go)[:50],
    ...
    def keyword(keyword):
        ...
        interpro = (
            Interpro.query.filter(
                or_(
                    Interpro.description.ilike("%" + keyword + "%"),
                    Interpro.label == keyword,
                )
            )
            .limit(50)
            .all()
        ...
        profiles = (
            ExpressionProfile.query.filter(ExpressionProfile.probe == keyword)
            .limit(50)
            .all()
        )
    ...
    def advanced_sequence_search(
        ...
        if gene_family_method_id > 0:
            query = query.filter(
                Sequence.families.any(
                    or_(
                        *[
                            and_(
                                GeneFamily.method_id == gene_family_method_id,
                                GeneFamily.name == name,
                            )
                            for name in gene_families
                        ]
                    )
                )
            )
        ...
        if len(gene_list) > 0:
            query = query.filter(
                or_(
                    *[
                        or_(Sequence.name == gene, Sequence.xrefs.any(name=gene))
        ...
        if len(terms.strip()) > 5:
            if term_rules == "exact":
                ...
                whooshee_search_terms = [
                    t for t in re.sub("(\W|\d)+", " ", terms).split() if len(t) > 3
                ]
                whooshee_search_string = " ".join(whooshee_search_terms)
        ...
        if go_terms is not None and len(go_terms) > 0:
            selected_go_id = [
                go.id for go in GO.query.filter(GO.name.in_(go_terms)).all()
            ]
            ...
            if go_rules == "all":
                if include_predictions:
                    query = query.filter(
                        and_(
                            *[
                                Sequence.go_associations.any(go_id=go_id)
                    ...
                    query = query.filter(
                        and_(
                            *[
                                Sequence.go_associations.any(go_id=go_id, predicted=0)
                                for go_id in selected_go_id
                            ]
                        )
                    )
        ...
        if interpro_domains is not None and len(interpro_domains) > 0:
            if interpro_rules == "all":
                query = query.filter(
                    and_(
                        *[
                            Sequence.interpro_domains.any(description=t)
                            for t in interpro_domains
                        ]
                    )
                )
            else:
    ...
    def count_enriched_clusters(
        ...
        if method != -1:
            clusters = clusters.filter(
                ClusterGOEnrichment.cluster.has(method_id=method)
            )
        ...
        if max_corrected_p is not None:
            clusters = clusters.filter(
                ClusterGOEnrichment.corrected_p_value <= max_corrected_p
            )

        if enriched_clade_id is not None:
            clusters = clusters.filter(
                ClusterGOEnrichment.cluster.has(
