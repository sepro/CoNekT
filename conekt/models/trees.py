    ...
    def reconcile_trees(self):
        ...
        for t in self.trees:
            ...
            for node in tree.walk():
                ...
                duplication = phylo.is_duplication(
                    branch_one_species, branch_two_species, clade_to_species
                )
        ...
        db.engine.execute(
            SequenceSequenceCladeAssociation.__table__.insert(), new_associations
        )
...
class Tree(db.Model):
    ...
    gf_id = db.Column(
        db.Integer, db.ForeignKey("gene_families.id", ondelete="CASCADE"), index=True
    )
    method_id = db.Column(
        db.Integer, db.ForeignKey("tree_methods.id", ondelete="CASCADE"), index=True
    )
    ...
    @staticmethod
    def __yattag_node(
        node, tag, text, line, id_to_clade, seq_to_species, seq_to_id, root=1
    ):
        with tag("clade"):
            if root == 1:
