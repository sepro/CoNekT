...
class GeneFamilyMethod(db.Model):
    ...
    tree_methods = db.relationship(
        "TreeMethod",
        backref=db.backref("gf_method", lazy="joined"),
        lazy="dynamic",
        cascade="all, delete-orphan",
        passive_deletes=True,
    )
    ...
    def get_interpro_annotation(self):
        ...
        for f in families:
            ...
            if len(sequences) > 4:
                for s in sequences:
                    interpro_ids = list(
                        set([i.interpro_id for i in s.interpro_associations])
                    )
    ...
    def get_go_annotation(self):
        ...
        for f in families:
            ...
            data = (
                SequenceGOAssociation.query.filter(SequenceGOAssociation.predicted == 0)
                .filter(SequenceGOAssociation.evidence is not None)
                .join(subquery, SequenceGOAssociation.sequence_id == subquery.c.id)
                .all()
            )
...
class GeneFamily(db.Model):
    ...
    method_id = db.Column(
        db.Integer,
        db.ForeignKey("gene_family_methods.id", ondelete="CASCADE"),
        index=True,
    )
    ...
    original_name = db.Column(
        db.String(50, collation=SQL_COLLATION), index=True, default=None
    )
    ...
    interpro_domains = db.relationship(
        "Interpro", secondary=family_interpro, lazy="dynamic"
    )
    ...
    def ecc_associations(self):
        ...
        output = (
            SequenceSequenceECCAssociation.query.filter_by(
                gene_family_method_id=self.method_id
            )
            .filter(
                or_(
                    or_(
                        *[
                            SequenceSequenceECCAssociation.query_id == s
                            for s in sequence_ids
                        ]
                    ),
                    or_(
                        *[
                            SequenceSequenceECCAssociation.target_id == s
                            for s in sequence_ids
                        ]
                    ),
                )
    ...
    def ecc_associations_paginated(self, page=1, page_items=30):
        ...
        output = (
            ...
            .paginate(page, page_items, False)
            .items
        )
    ...
    def sequence_stats(sequence_ids):
        ...
        """
        data = SequenceFamilyAssociation.query.filter(
    ...
    def sequence_stats_subquery(sequences):
        ...
        data = SequenceFamilyAssociation.query.join(
            subquery, SequenceFamilyAssociation.sequence_id == subquery.c.id
        ).all()
    ...
    @staticmethod
    def add_families_from_mcl(
        filename, description, handle_isoforms=False, prefix="mcl"
    ):
        """
    ...
    def add_families_from_orthofinder(filename, description, handle_isoforms=False):
        ...
        with open(filename, "r") as f_in:
            for line in f_in:
                ...
                new_family = GeneFamily(
                    orthofinder_id.replace("OG", "OG_%02d_" % method.id)
                )
