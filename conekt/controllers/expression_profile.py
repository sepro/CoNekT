...
from conekt.helpers.chartjs import (
    prepare_expression_profile,
    prepare_profile_comparison,
)
...
def expression_profile_view(profile_id):
    ...
    expression_specificity_methods = ExpressionSpecificityMethod.query.filter(
        ExpressionSpecificityMethod.species_id == current_profile.species_id
    ).all()
    ...
    return render_template(
        "expression_profile.html", profile=current_profile, tissues=tissues
    )
...
def expression_profile_find(probe, species_id=None):
    ...
    return redirect(
        url_for(
            "expression_profile.expression_profile_view", profile_id=first_profile.id
        )
    )
...
@expression_profile.route(
    "/compare/<first_profile_id>/<second_profile_id>/<int:normalize>"
)
@cache.cached()
def expression_profile_compare(first_profile_id, second_profile_id, normalize=0):
    ...
    networks = ExpressionNetwork.query.filter(
        ExpressionNetwork.sequence_id == first_profile.sequence_id
    ).all()
    ...
    for n in networks:
        ...
        for link in data:
            if (
                "gene_id" in link.keys()
                and link["gene_id"] == second_profile.sequence_id
            ):
                if "link_pcc" in link.keys():
                    pcc = (
                        link["link_pcc"]
                        if pcc is None or pcc < link["link_pcc"]
                        else pcc
                    )
                if "hrr" in link.keys():
...
@expression_profile.route(
    "/compare_probes/<probe_a>/<probe_b>/<int:species_id>/<int:normalize>"
)
@cache.cached()
def expression_profile_compare_probes(probe_a, probe_b, species_id, normalize=0):
    ...
    """
    first_profile = (
        ExpressionProfile.query.filter_by(probe=probe_a)
        .filter_by(species_id=species_id)
        .first_or_404()
    )
    second_profile = (
        ExpressionProfile.query.filter_by(probe=probe_b)
        .filter_by(species_id=species_id)
        .first_or_404()
    )
...
def expression_profile_download_plot(profile_id):
    ...
    """
    current_profile = ExpressionProfile.query.options(undefer("profile")).get_or_404(
        profile_id
    )
...
@expression_profile.route(
    "/json/compare_plot/<first_profile_id>/<second_profile_id>/<int:normalize>"
)
@cache.cached()
def expression_profile_compare_plot_json(
    first_profile_id, second_profile_id, normalize=0
):
    """
    ...
    :return:
    """
    first_profile = ExpressionProfile.query.options(undefer("profile")).get_or_404(
        first_profile_id
    )
    second_profile = ExpressionProfile.query.options(undefer("profile")).get_or_404(
        second_profile_id
    )
...
def __generate(species_id, method_id, condition):
    ...
    profiles = (
        ExpressionProfile.query.filter(ExpressionProfile.species_id == species_id)
        .filter(ExpressionProfile.sequence_id is not None)
        .options(undefer("profile"))
        .order_by(ExpressionProfile.probe.asc())
        .all()
    )

    condition_tissue = ConditionTissue.query.filter(
        ConditionTissue.expression_specificity_method_id == method_id
    ).first()

    condition_tissue_data = (
        json.loads(condition_tissue.data) if condition_tissue is not None else None
    )
    ...
    for p in profiles:
        try:
            ...
            if condition_tissue is None:
                ...
                converted_profile = ExpressionProfile.convert_profile(
                    condition_tissue_data, data, use_means=True
                )
                values = converted_profile["data"][condition]

            aliases = p.sequence.aliases if p.sequence.aliases is not None else ""
            description = (
                p.sequence.description if p.sequence.description is not None else ""
            )
            ...
            print(
                "An error occured exporting a profile with conditions %s for species %d."
...
def export_expression_levels():
    ...
    if request.method == "POST":
        ...
        _, filepath = tempfile.mkstemp(
            prefix="expr_", dir=current_app.config["TMP_DIR"]
        )
        ...
        return Response(
            json.dumps(
                {
                    "url": url_for(
                        "expression_profile.export_expression_levels_file",
                        name=filename,
                    )
                }
            ),
