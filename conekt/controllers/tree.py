...
from conekt.models.relationships.sequence_sequence_clade import (
...
def tree_view(tree_id):
    ...
    association_count = SequenceSequenceCladeAssociation.query.filter(
        SequenceSequenceCladeAssociation.tree_id == tree_id
    ).count()
...
def tree_associations(tree_id, page=1):
    relations = (
        SequenceSequenceCladeAssociation.query.filter(
            SequenceSequenceCladeAssociation.tree_id == tree_id
        )
        .paginate(page, g.page_items, False)
        .items
...
def tree_associations_table(tree_id):
    relations = SequenceSequenceCladeAssociation.query.filter(
        SequenceSequenceCladeAssociation.tree_id == tree_id
    ).all()

    return Response(
        render_template("tables/clade_associations.csv", relations=relations),
        mimetype="text/plain",
    )
...
def newick(tree_id):
    ...
    response.headers["Content-Disposition"] = (
        "attachment; filename=" + tree.label + ".newick"
    )
...
def newick_no_branch_lengths(tree_id):
    ...
    response.headers["Content-Disposition"] = (
        "attachment; filename=" + tree.label + ".no_branch_lengths.newick"
    )
    response.headers["Content-type"] = "text/plain"
...
def ascii_tree(tree_id):
    ...
    response.headers["Content-Disposition"] = (
        "attachment; filename=" + tree.label + ".ascii.txt"
    )
    response.headers["Content-type"] = "text/plain"
...
def phyloxml(tree_id):
    ...
    response.headers["Content-Disposition"] = (
        "attachment; filename=" + tree.label + ".xml"
    )
