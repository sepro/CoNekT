...
def blast_main():
    ...
    sequence = (
        Sequence.query.filter(Sequence.type == "protein_coding")
        .filter(func.length(Sequence.coding_sequence) > 300)
        .first()
    )
...
def blast_results_json(token):
    ...
    if os.path.exists(file_results) and os.access(file_results, os.W_OK):
        ...
        return Response(
            json.dumps({"status": "done", "data": output}), mimetype="application/json"
        )
    else:
        if not os.path.exists(file_in):
            ...
            return Response(
                json.dumps({"status": "waiting"}), mimetype="application/json"
            )
