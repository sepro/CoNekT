...
def interpro_view(interpro_id):
    ...
    return render_template(
        "interpro.html", interpro=current_interpro, count=sequence_count
    )
...
def interpro_sequences(interpro_id, page=1):
    ...
    """
    sequences = (
        Interpro.query.get(interpro_id)
...
def interpro_interpro_ajax(interpro_id):
    ...
    return render_template(
        "async/interpro_stats.html",
        interpro_stats={
            k: v
            for k, v in current_interpro.interpro_stats.items()
            if str(k) != str(interpro_id)
        },
    )
...
def interpro_family_ajax(interpro_id):
    ...
    return render_template(
        "async/family_stats.html", family_stats=current_interpro.family_stats
    )
