...
def search_single_keyword(keyword):
    ...
    if (
        len(results["sequences"])
        + len(results["go"])
        + len(results["interpro"])
        ...
        if len(results["sequences"]) == 1:
            return redirect(
                url_for(
                    "sequence.sequence_view", sequence_id=results["sequences"][0].id
                )
            )
            ...
            return redirect(
                url_for("interpro.interpro_view", interpro_id=results["interpro"][0].id)
            )
        elif len(results["families"]) == 1:
            return redirect(
                url_for("family.family_view", family_id=results["families"][0].id)
            )
        elif len(results["profiles"]) == 1:
            return redirect(
                url_for(
                    "expression_profile.expression_profile_view",
    ...
    if not g.search_form.validate_on_submit():
        ...
        if (
            ...
            if len(results["sequences"]) == 1:
                ...
                return redirect(
                    url_for(
                        "interpro.interpro_view", interpro_id=results["interpro"][0].id
                    )
                )
            elif len(results["families"]) == 1:
                return redirect(
        ...
        return render_template(
            "search_results.html",
            keyword=g.search_form.terms.data,
            go=results["go"],
            interpro=results["interpro"],
            sequences=results["sequences"],
            families=results["families"],
...
def advanced():
    ...
    if request.method == "GET":
        return render_template(
            "search_advanced.html", adv_sequence_form=adv_sequence_form
        )
    else:
        ...
        go_terms = [
            gt.data["go_term"]
            for gt in adv_sequence_form.go_terms.entries
            if gt.data["go_term"] != ""
        ]
        ...
        interpro_terms = [
            it.data["interpro_domain"]
            for it in adv_sequence_form.interpro_domains.entries
            if it.data["interpro_domain"] != ""
        ]

        results = Search.advanced_sequence_search(
            species_id,
        ...
        return render_template(
            "search_results.html",
            keyword="Advanced search results",
            sequences=results,
            advanced=True,
        )
...
def count_enriched_clusters():
    ...
    try:
        ...
        max_corrected_p = (
            float(content["max_corrected_p"]) if check_corrected_p else None
        )

        enable_clade_enrichment = bool(content["enable_clade_enrichment"])
        clade = int(content["clade"]) if enable_clade_enrichment else None

        go = GO.query.filter(or_(GO.name == term, GO.label == term)).first()

        cluster_count = Search.count_enriched_clusters(
            go.id,
            method=method,
        ...
        return Response(
            json.dumps({"count": cluster_count, "error": 0}),
            mimetype="application/json",
        )
    except Exception as e:
        # Bad data return zero
        return Response(
            json.dumps({"count": 0, "error": 1}), mimetype="application/json"
        )
...
def search_enriched_clusters():
    ...
    if request.method == "POST":
        ...
        min_enrichment = (
            request.form.get("min_enrichment") if check_enrichment else None
        )
        max_p = request.form.get("max_p") if check_p else None
        max_corrected_p = (
            request.form.get("max_corrected_p") if check_corrected_p else None
        )
        ...
        go = GO.query.filter(or_(GO.name == term, GO.label == term)).all()

        results = []

        for g in go:
            clusters = Search.enriched_clusters(
                g.id,
                method=method,
                min_enrichment=min_enrichment,
                max_p=max_p,
        ...
        example = {"go_term": None, "method": None, "max_corrected_p": "0.05"}

        enrichment = ClusterGOEnrichment.query.filter(
            ClusterGOEnrichment.corrected_p_value < 0.05
        ).first()
        ...
        return render_template(
            "find_enriched_clusters.html", form=form, example=example
        )
    ...
    try:
        ...
        count = (
            ExpressionSpecificity.query.filter(
                ExpressionSpecificity.method_id == method
            )
        ...
        return Response(
            json.dumps({"count": count, "error": 0}), mimetype="application/json"
        )
    except Exception as e:
        # Bad data return zero
        return Response(
...
def search_specific_profiles():
    ...
    if request.method == "GET":
        ...
        results = (
            ExpressionSpecificity.query.filter(
                ExpressionSpecificity.method_id == method_id
            )
            .filter(ExpressionSpecificity.score >= cutoff)
            .filter(ExpressionSpecificity.condition == condition)
            .options(joinedload(ExpressionSpecificity.profile).undefer("profile"))
        )

        return render_template(
            "find_specific_profiles.html",
            results=results,
            species=species,
            method=method,
            condition=condition,
...
def search_specific_profiles_json():
    ...
    methods = ExpressionSpecificityMethod.query.order_by(
        ExpressionSpecificityMethod.menu_order
    ).all()
...
def search_specific_profiles_methods(species_id):
    ...
    """
    methods = (
        ExpressionSpecificityMethod.query.filter_by(species_id=species_id)
        .order_by(ExpressionSpecificityMethod.menu_order)
        .all()
    )
...
def search_typeahead_interpro(term):
    ...
    if len(term) > 7:
        interpro = (
            Interpro.query.filter(
                or_(
                    Interpro.description.ilike("%" + term + "%"),
                    Interpro.label.ilike(term + "%"),
                )
            )
            .order_by(func.length(Interpro.description))
            .all()
        )
    else:
        interpro = (
            Interpro.query.filter(Interpro.description.ilike("%" + term + "%"))
            .order_by(func.length(Interpro.description))
            .all()
        )
...
def search_typeahead_prefetch_interpro():
    ...
    """
    interpro = (
        Interpro.query.filter(func.length(Interpro.description) < 7)
        .order_by(func.length(Interpro.description))
        .all()
    )
...
def search_typeahead_go(term):
    ...
    if term.lower().startswith("go:") and len(term) > 7:
        ...
        go = (
            GO.query.filter(GO.obsolete == 0)
            .filter(GO.name.ilike("%" + term + "%"))
            .order_by(func.length(GO.name))
            .all()
        )
