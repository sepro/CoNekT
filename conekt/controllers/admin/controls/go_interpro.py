...
def add_go():
    ...
    if request.method == "POST":
        ...
        if file != b"":
            ...
            flash(
                "Empty file or no file provided, cannot add GO terms to sequences",
                "warning",
            )
...
def add_interpro():
    ...
    if request.method == "POST":
        ...
        if file != b"":
            ...
            flash(
                "Empty file or no file provided, cannot add InterPro terms to sequences",
                "warning",
            )
...
def calculate_enrichment():
    ...
    try:
        ...
        flash(
            "Successfully calculated GO enrichment for co-expression clusters",
            "success",
        )
...
def predict_from_network():
    ...
    if request.method == "POST":
        ...
        try:
            GO.predict_from_network_enrichment(
                network_method_id, cutoff=p_cutoff, source=description
            )
            flash("Predicted GO terms from network %d" % network_method_id, "success")
        except Exception as e:
            print(e)
            flash(
                "Failed to predicted GO terms from network %d" % network_method_id,
                "danger",
            )
