...
from conekt.forms.admin.add_expression_specificity import (
    AddConditionSpecificityForm,
    AddTissueSpecificityForm,
)
...
def add_condition_specificity():
    ...
    if request.method == "POST" and form.validate():
        ...
        ExpressionSpecificityMethod.calculate_specificities(
            species_id, description, False
        )

        flash(
            "Calculated condition specificities for species %d" % species_id, "success"
        )
...
def add_tissue_specificity():
    ...
    if request.method == "POST" and form.validate():
        ...
        if file != b"":
            ...
            ConditionTissue.add(
                species_id, condition_tissue, order, colors, specificity_method_id
            )
