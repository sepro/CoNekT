...
def expression_cluster_overview():
    ...
    return render_template(
        "expression_cluster.html", cluster_methods=cluster_methods, overview=True
    )
...
def expression_cluster_view(cluster_id):
    ...
    similar_clusters = CoexpressionClusterSimilarity.query.filter(
        or_(
...
def expression_cluster_sequences(cluster_id, page=1):
    ...
    sequence_associations = (
        cluster.sequence_associations.order_by(
            SequenceCoexpressionClusterAssociation.probe
        )
    ...
    return render_template(
        "pagination/cluster_probes.html",
        sequence_associations=sequence_associations,
...
def expression_cluster_download(cluster_id):
    ...
    """
    cluster = CoexpressionCluster.query.get_or_404(cluster_id)
    sequence_associations = cluster.sequence_associations.order_by(
        SequenceCoexpressionClusterAssociation.probe
    )
...
def cluster_interpro_ajax(cluster_id):
    ...
    return render_template(
        "async/interpro_stats.html", interpro_stats=current_cluster.interpro_stats
    )
...
def cluster_family_ajax(cluster_id):
    ...
    return render_template(
        "async/family_stats.html", family_stats=current_cluster.family_stats
    )
